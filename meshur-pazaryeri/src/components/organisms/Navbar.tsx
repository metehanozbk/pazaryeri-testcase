...
export const Navbar = () => {
  const t = useTranslations('Index'); // t artık kullanılıyor
    ...
    <nav className="sticky top-0 z-50 border-b bg-white/80 backdrop-blur-md dark:bg-zinc-950/80">
      <div className="container mx-auto flex h-16 items-center justify-between px-4">
        ...
        <div className="flex items-center gap-4">
          <Link href="/favorites" className="relative p-2 text-zinc-600 transition-colors hover:text-orange-500 dark:text-zinc-300">
          ...
          {/* t kullanım örneği */}
          <span className="hidden text-sm sm:block">{t('welcome') || ''}</span>
        </div>
      </div>
    </nav>
  );
